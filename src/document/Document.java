...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		List<String> vowelGroups = new ArrayList<String>();
		Pattern tokSplitter = Pattern.compile("[aeiouyAEIOUY]+");
		Matcher m = tokSplitter.matcher(word);
		while (m.find()) {
			vowelGroups.add(m.group());
		}
		int numSyllables = vowelGroups.size();
		if (numSyllables>1 && vowelGroups.get(numSyllables-1).equals("e") && word.charAt(word.length()-1)=='e')
			numSyllables--;
	...
	public double getFleschScore()
	{
	    ...
	    System.out.println("NumSyllables per word:" + numSyllables/numWords);
	    System.out.println("NumSentences:" + numSentences); */
	    
	    return 206.835d-1.015d*(numWords/numSentences)-84.6*(numSyllables/numWords);
